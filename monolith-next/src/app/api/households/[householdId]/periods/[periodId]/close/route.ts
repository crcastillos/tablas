...
import { closePeriod } from "@/lib/backend/services";
...
export async function POST(request: NextRequest, context: Context) {
  return withErrorHandling(async () => {
    ...
    const data = await closePeriod(householdId, periodId, auth.userId);
    return NextResponse.json(ok(data, "Período cerrado."));
  });
}

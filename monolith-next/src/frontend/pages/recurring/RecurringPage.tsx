...
import { categoriesApi, recurringApi } from "@/frontend/api/budgets.api";
import type { ExpenseCategory, RecurringExpense } from "@/frontend/types/domain";
...
import { RecurrenceFrequency } from "@/frontend/types/domain";
import { todayIso } from "@/frontend/utils/dates";
...
export default function RecurringPage() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const [cats, recurring] = await Promise.all([categoriesApi.list(activeHousehold.id), recurringApi.list(activeHousehold.id)]);
  ...
  return (
    <>
      ...
      <Paper>
        <Table>
          <TableHead><TableRow><TableCell>Nombre</TableCell><TableCell>Categoria</TableCell><TableCell align="right">Monto</TableCell><TableCell>Dia</TableCell><TableCell>Activo</TableCell></TableRow></TableHead>
          <TableBody>
            {items.map((item) => (
      ...
      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent sx={{ display: "grid", gap: 2, pt: 1 }}>
          ...
          <TextField label="Dia de pago" type="number" value={form.expectedPaymentDay} onChange={(e) => setForm({ ...form, expectedPaymentDay: Number(e.target.value) })} />

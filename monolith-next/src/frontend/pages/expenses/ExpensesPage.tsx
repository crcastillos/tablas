...
import { expensesApi } from "@/frontend/api/expenses.api";
import type { Expense, ExpenseCategory } from "@/frontend/types/domain";
import { EmptyState, ErrorState, LoadingState, PageHeader } from "@/frontend/components/ui/States";
import { formatCurrency } from "@/frontend/utils/currency";
import { formatDate, todayIso } from "@/frontend/utils/dates";
import { getErrorMessage } from "@/frontend/utils/getErrorMessage";
import { ExpenseMovementType, ExpenseStatus, PaymentMethodType, PeriodStatus } from "@/frontend/types/domain";
import { expenseStatusLabel, paymentMethodOptions } from "@/frontend/utils/labels";
...
export default function ExpensesPage() {
  ...
  return (
    <>
      <PageHeader title="Gastos y pagos" action={!readonly ? <Button variant="contained" onClick={handleOpen}>Registrar gasto</Button> : undefined} />
      ...
      <Paper>
        <Table>
          <TableHead><TableRow><TableCell>Fecha</TableCell><TableCell>Descripcion</TableCell><TableCell>Categoria</TableCell><TableCell align="right">Monto</TableCell><TableCell>Estado</TableCell></TableRow></TableHead>
          <TableBody>
            {items.map((expense) => (
      ...
      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent sx={{ display: "grid", gap: 2, pt: 1 }}>
          ...
          <TextField select label="Metodo de pago" value={form.paymentMethod} onChange={(e) => setForm({ ...form, paymentMethod: Number(e.target.value) as PaymentMethodType })}>
            {paymentMethodOptions.map(([label, value]) => <MenuItem key={value} value={value}>{label}</MenuItem>)}
          </TextField>
          {confirmUnbudgeted || confirmExceeded ? (
            <Alert severity="warning">
              {confirmUnbudgeted ? "La categoria no tiene presupuesto. " : ""}

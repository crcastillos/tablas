...
import { householdsApi, periodsApi } from "@/frontend/api/households.api";
import type { FinancialPeriod, Household } from "@/frontend/types/domain";
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const setActiveHousehold = useCallback((household: Household | null) => {
    ...
    if (household) window.localStorage.setItem(HOUSEHOLD_KEY, household.id);
  ...
  const setActivePeriod = useCallback((period: FinancialPeriod | null) => {
    ...
    if (typeof window === "undefined") return;
    if (period) window.localStorage.setItem(PERIOD_KEY, period.id);
    else window.localStorage.removeItem(PERIOD_KEY);
  }, []);
  ...
  const refreshHouseholds = useCallback(async () => {
    ...
    try {
      ...
      setHouseholds(list);
      const savedId = typeof window !== "undefined" ? window.localStorage.getItem(HOUSEHOLD_KEY) : null;
  ...
  const refreshPeriods = useCallback(async () => {
    ...
    setPeriods(list);
    const savedId = typeof window !== "undefined" ? window.localStorage.getItem(PERIOD_KEY) : null;

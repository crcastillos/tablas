...
export const expenseCategoryTypeOptions = Object.entries(ExpenseCategoryType).filter(([key]) => Number.isNaN(Number(key))) as [string, ExpenseCategoryType][]
export const paymentMethodOptions = Object.entries(PaymentMethodType).filter(([key]) => Number.isNaN(Number(key))) as [string, PaymentMethodType][]
export const expenseMovementOptions = Object.entries(ExpenseMovementType).filter(([key]) => Number.isNaN(Number(key))) as [string, ExpenseMovementType][]

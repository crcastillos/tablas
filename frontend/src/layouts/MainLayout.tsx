...
const navItems = [
  ...
  { to: '/periods', label: 'Período', icon: <CalendarMonthIcon /> },
  ...
  { to: '/categories', label: 'Categorías', icon: <CategoryIcon /> },
...
export default function MainLayout() {
  ...
  const drawer = (
    <Box sx={{ width: 260, display: 'flex', flexDirection: 'column', height: '100%' }} role="navigation" aria-label="Menú principal">
      <Box sx={{ p: 2 }}>
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh' }}>
      <AppBar position="fixed" sx={{ zIndex: (t) => t.zIndex.drawer + 1 }}>
        <Toolbar sx={{ gap: 2, flexWrap: 'wrap' }}>
          {mobile ? (
            <IconButton color="inherit" edge="start" onClick={() => setOpen(true)} aria-label="Abrir menú">
              <MenuIcon />
            </IconButton>
          ) : null}
          <Typography variant="h6" sx={{ flexGrow: { xs: 1, md: 0 } }}>{import.meta.env.VITE_APP_NAME ?? 'Finanzas del Hogar'}</Typography>
          <Select
            size="small"
          ...
          >
            <MenuItem value="" disabled>Período</MenuItem>
            {periods.map((p) => (
          ...
          <IconButton color="inherit" aria-label="Registrar gasto rápido" onClick={() => navigate('/expenses?quick=1')}>
            <AddIcon />
          </IconButton>
          <Button
            color="inherit"

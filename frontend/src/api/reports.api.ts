...
export const reportsApi = {
  ...
  budgetVsActual: async (householdId: string, periodId: string) => {
    const { data } = await apiClient.get<ApiResponse<BudgetVsActualReport>>(`/households/${householdId}/periods/${periodId}/reports/budget-vs-actual`)
    return data.data!
  },
  ...
  monthlyComparison: async (householdId: string, months = 6) => {
    const { data } = await apiClient.get<ApiResponse<MonthlyComparisonReport>>(`/households/${householdId}/reports/monthly-comparison`, { params: { months } })
    return data.data!
  },
}

...
import type { ApiResponse, Income, IncomeType } from '../types/domain'
...
export const incomesApi = {
  ...
  update: async (householdId: string, incomeId: string, payload: Record<string, unknown>) => {
    const { data } = await apiClient.put<ApiResponse<Income>>(`/households/${householdId}/incomes/${incomeId}`, payload)
    return data.data!
  },
  ...
  cancel: async (householdId: string, incomeId: string, reason: string) => {
    const { data } = await apiClient.post<ApiResponse<Income>>(`/households/${householdId}/incomes/${incomeId}/cancel`, { reason })
    return data.data!
  },
}
